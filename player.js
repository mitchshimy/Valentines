...
audioPlayer.addEventListener('play', () => {
  isPlaying = true;
  updatePlayPauseIcon();
  setTimeout(prefetchNextTrackSafely, 1200);
});
...
function prefetchNextTrackSafely() {
if (!('serviceWorker' in navigator)) return;
if (!navigator.serviceWorker.controller) return;
if (!isPowerOn || !isPlaying) return;

// Mobile safety: prefetch only on Wi-Fi or undefined
...
async function playSong(force = false) {
  ...
  // Same track but paused → just play
  if (!force && sameTrack && audioPlayer.paused) {
    audioPlayer.play().catch(() => {});
    return;
  }

  // Different track or force → load
  if (!sameTrack || force) {
    audioPlayer.pause();
...
function setTrackById(trackId, { forcePlay = false, resetTime = true } = {}) {
  ...
  // Notify SW about preferred music for background caching/prefetch
...
function shuffleSongs() {
  ...
  // Keep the current track ID so we can continue playing it
  const currentlyPlayingId = currentTrackId;

  // Shuffle displayOrder only
...
//prefetch the next track in cache
  if (isPlaying && isPowerOn) prefetchNextTrackSafely();
